...
BEGIN
	...
	copy tmp_malformed from '/home/gpadmin/POSTGRESQL/Checkbook_ogent/KETTLE_JOB/PreProcessing_DataFiles/badfile.txt';
		...
		LOOP
			...
			IF l_rule.rule_name  IN ('Duplicate', 'Multiple') THEN
					...
					l_select_str := 'INSERT INTO tmp_duplicates(all_uniq_id,min_uniq_id) ' ||
							' SELECT group_concat(uniq_id::varchar) as all_uniq_id, min(uniq_id) as min_uniq_id ' ||
...
BEGIN
	...
	FROM etl.etl_data_load_file c JOIN
		...
		WHERE 	b.job_id = p_job_id_in
			AND b.data_source_code IN ('A','D','E','L','O','RC','RY','RS','BC','FC','V','PC','ED','TD')
...
BEGIN
	...
	l_start_time := timeofday()::timestamp;
	l_status := etl.restoreOGETransactionsData(p_job_id_in) ;
/*	
...
select load_file_id,count(*) as total_invalid_records,invalid_reason as invalid_reason from etl.invalid_payroll_summary group by 1,3 union 
select load_file_id,count(*) as total_invalid_records,invalid_reason as invalid_reason from etl.invalid_edc_contract group by 1,3;
