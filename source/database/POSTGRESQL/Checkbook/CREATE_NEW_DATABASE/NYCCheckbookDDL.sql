...
CREATE TABLE agreement_snapshot_cy (LIKE agreement_snapshot);
...
CREATE TABLE deleted_agreement_accounting_line (LIKE history_agreement_accounting_line) ;
...
-- agreement_snapshot, agreement_snapshot_cy, agreement_snapshot_expanded, agreement_snapshot_expanded_cy, disbursement_line_item_details and pending_contracts table changes
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 -- Indexes for distributed columns that are not primary keys
 
  CREATE INDEX idx_payroll_id_payroll_future_data   ON  payroll_future_data(payroll_id)  ;
  CREATE INDEX idx_employee_id_agg_on_payroll_emp_agency   ON   aggregateon_payroll_employee_agency(employee_id) ;
  CREATE INDEX idx_agency_id_agg_on_payroll_agency   ON   aggregateon_payroll_agency(agency_id) ;
  CREATE INDEX idx_agency_id_agg_on_payroll_coa_month   ON   aggregateon_payroll_coa_month(agency_id) ;
  CREATE INDEX idx_fiscal_year_id_agg_on_payroll_year   ON   aggregateon_payroll_year(fiscal_year_id) ;
  CREATE INDEX idx_fiscal_year_id_agg_on_payroll_year_month   ON   aggregateon_payroll_year_and_month(fiscal_year_id) ;
  CREATE INDEX idx_employee_id_agg_on_payroll_employee_agency_month   ON  aggregateon_payroll_employee_agency_month(employee_id)  ;
  CREATE INDEX idx_agency_id_agg_on_payroll_agency_month   ON   aggregateon_payroll_agency_month(agency_id) ;
 
 
  CREATE INDEX idx_vendor_customer_code_fmsv_bus_type   ON    fmsv_business_type(vendor_customer_code);
  CREATE INDEX idx_agreement_id_hist_agg_commodity   ON    history_agreement_commodity(agreement_id);
  CREATE INDEX idx_agreement_id_hist_agg_worksite   ON    history_agreement_worksite(agreement_id);
  CREATE INDEX idx_original_agreement_id_agr_snapshot   ON    agreement_snapshot(original_agreement_id);
  CREATE INDEX idx_original_agreement_id_agr_snapshot_cy   ON    agreement_snapshot_cy(original_agreement_id);
  CREATE INDEX idx_agreement_id_hist_agr_acc_line   ON    history_agreement_accounting_line(agreement_id);
    CREATE INDEX idx_agreement_id_agreement_snapshot_deleted   ON  agreement_snapshot_deleted(agreement_id)  ;
   CREATE INDEX idx_agreement_id_agreement_snapshot_cy_deleted   ON  agreement_snapshot_cy_deleted(agreement_id)  ;
   CREATE INDEX idx_agreement_id_deleted_agreement_acc_line   ON  deleted_agreement_accounting_line(agreement_id)  ;
   CREATE INDEX idx_document_code_id_pending_contracts   ON  pending_contracts(document_code_id) ;
   CREATE INDEX idx_original_agreement_id_agreement_snap_expanded   ON  agreement_snapshot_expanded(original_agreement_id) ;
  CREATE INDEX idx_original_agreement_id_agreement_snap_expanded_cy   ON  agreement_snapshot_expanded_cy(original_agreement_id) ;
  CREATE INDEX idx_original_agreement_id_mid_agg_on_disb_spe_year   ON  mid_aggregateon_disbursement_spending_year(original_agreement_id) ;
  CREATE INDEX idx_vendor_id_agg_on_contracts_cumulative_spending   ON  aggregateon_contracts_cumulative_spending(vendor_id)  ;
  CREATE INDEX idx_vendor_id_agg_on_contracts_spending_by_month   ON  aggregateon_contracts_spending_by_month(vendor_id)  ;
  CREATE INDEX idx_fiscal_year_agg_on_total_contracts   ON   aggregateon_total_contracts(fiscal_year) ;
  CREATE INDEX idx_department_id_agg_on_contracts_department   ON   aggregateon_contracts_department(department_id) ;
  CREATE INDEX idx_disbursement_line_item_id_con_spe_transactions   ON   contracts_spending_transactions(disbursement_line_item_id) ;
  CREATE INDEX idx_original_agreement_id_agg_on_contracts_expense   ON  aggregateon_contracts_expense(original_agreement_id)  ;
  
  
  CREATE INDEX idx_disbursement_line_item_id_disb_line_item_deleted   ON    disbursement_line_item_deleted(disbursement_line_item_id);
  CREATE INDEX idx_disbursement_line_item_id_disb_line_item_details   ON    disbursement_line_item_details(disbursement_line_item_id);
  CREATE INDEX idx_department_id_agg_on_spe_coa_entities   ON   aggregateon_spending_coa_entities(department_id) ;
  CREATE INDEX idx_agreement_id_agg_on_spe_contract   ON   aggregateon_spending_contract(agreement_id) ;
  CREATE INDEX idx_agreement_id_agg_on_spe_vendor   ON   aggregateon_spending_vendor(vendor_id) ;
  CREATE INDEX idx_expenditure_object_id_agg_on_spe_ven_exp_object   ON   aggregateon_spending_vendor_exp_object(expenditure_object_id) ;

  
  CREATE INDEX idx_agency_id_agg_on_budget_by_year   ON  aggregateon_budget_by_year(agency_id)  ;  
   CREATE INDEX idx_revenue_id_revenue_details   ON    revenue_details(revenue_id);
   CREATE INDEX idx_agency_id_agg_on_rev_cat_funding_class   ON   aggregateon_revenue_category_funding_class(agency_id) ;
  CREATE INDEX idx_agency_id_agg_on_rev_cat_funding_by_year   ON   aggregateon_revenue_category_funding_by_year(agency_id) ;
  
   -- Indexes for distributed columns for mwbe aggregate tables that are not primary keys
  ...
  --  Indexes for distributed columns and others for subvendor tables
