...
--	(SELECT fms_contract_number, vendor_id, max(current_amount) as current_amount FROM oge_contract group by 1,2) edc_data
	...
	SET agency_history_id = edc_data.agency_history_id,
		agency_id = edc_data.agency_id,
		oge_agency_id = edc_data.agency_id,
	...
	FROM (select ha.agreement_id, ag.agency_code, ag.agency_id, ag.agency_name, ag.agency_short_name, agh.agency_history_id, original_amount, current_amount 
	FROM history_agreement ha 
	JOIN oge_contract_contract_level edc ON ha.contract_number = edc.fms_contract_number  
	JOIN ref_agency ag ON edc.agency_id = ag.agency_id 
	JOIN (select agency_id, max(agency_history_id) agency_history_id from ref_agency_history group by 1) agh ON ag.agency_id = agh.agency_id) edc_data 
	WHERE a.agreement_id = edc_data.agreement_id;
	...
	FROM (select ha.agreement_id, ag.agency_code, ag.agency_id, ag.agency_name, ag.agency_short_name, agh.agency_history_id, original_amount, current_amount  
	FROM history_agreement ha 
	JOIN  oge_contract_contract_level edc ON ha.contract_number = edc.fms_contract_number  JOIN ref_agency ag ON edc.agency_id = ag.agency_id 
	JOIN (select agency_id, max(agency_history_id) agency_history_id from ref_agency_history group by 1) agh ON ag.agency_id = agh.agency_id) edc_data 
	WHERE a.agreement_id = edc_data.agreement_id;
	*/
...
UPDATE agreement_snapshot a
SET has_children = 'Y'
FROM (select master_agreement_id, count(distinct original_agreement_id) FROM agreement_snapshot 
WHERE master_agreement_id IS NOT NULL group by 1 having count(distinct original_agreement_id) > 1) b
WHERE a.original_agreement_id = b.master_agreement_id AND a.master_agreement_yn = 'Y';
...
RAISE NOTICE 'UPDATE FMS DATA FOR OGE 4';
	...
	UPDATE agreement_snapshot a
	SET original_contract_amount = edc_data.original_contract_amount
	FROM (select master_agreement_id, sum(oge_registered_amount) as original_contract_amount FROM (select distinct contract_number, master_agreement_id from history_agreement) a, oge_contract b 	WHERE a.contract_number = b.fms_contract_number GROUP BY 1) edc_data 
	WHERE a.original_agreement_id = edc_data.master_agreement_id;
