...
namespace Drupal\data_controller\Datasource\Handler;
...
abstract class AbstractDataSourceHandler extends AbstractObject implements DataSourceHandler {
    ...
    public function getExtension($functionalityName) {
        ...
        $extensionClassName = $this->extensions[$functionalityName] ?? NULL;
    ...
    protected function prepareDataType4Casting($datatype) {
        if (isset($datatype)) {
            [$datasetName, $columnName] = ReferencePathHelper::splitReference($datatype);
            if (isset($datasetName)) {
                $adjustedDataType = $this->originalDataType4ReferencedDataTypes[$datatype] ?? NULL;
