...
use Drupal\data_controller\Datasource\Reference\ReferenceLink;
use Drupal\data_controller\Datasource\Reference\ReferenceLinkBuilder;
...
use Drupal\data_controller\Datasource\Request\Dataset\AbstractDatasetQueryRequest;
...
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\AssembledSections;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\ColumnSection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\CompositeColumnSection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\CompositeWhereConditionSection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\DatasetSection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\ExactConditionSectionValue;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\JoinConditionSection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\Statement;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\SubquerySection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\TableColumnConditionSectionValue;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\WhereConditionSection;
use Drupal\data_controller_sql\Datasource\Handler\Generator\CubeStatementGenerator;
...
use Drupal\data_controller_sql\Metamodel\DatasetTypeHelper;
...
abstract class AbstractSQLDataSourceQueryHandler extends AbstractSQLDataSourceHandler implements DataSourceQueryHandler{
  ...
  public function queryDataset(DataControllerCallContext $callcontext, DatasetQueryRequest $request, ResultFormatter $resultFormatter)
  {
    ...
    if (in_array($datasetName, $cacheDatasets)) {
      if ($result = _checkbook_dmemcache_get($cacheKey)) {
        LogHelper::log_info("Memcached key matches. Loading cached result. Key " . $cacheKey . " DatasetName: $datasetName");
        LogHelper::log_info("Memcached SQL: " . $sql);
        return $result;
      }
    }
    //@ToDo
    StatementLogMessageKeeper::$statements['dataset.query'][] = preg_replace('/\s+/', ' ', str_replace("\r","",str_replace("\n","",$sql)));

    $result = $this->executeQuery($callcontext, $datasource, $sql, $resultFormatter);
    if (in_array($datasetName, $cacheDatasets)) {
      _checkbook_dmemcache_set($cacheKey, $result);

      LogHelper::log_info("Setting memcache with cachekey " . $cacheKey . " DatasetName: $datasetName");
      LogHelper::log_info("Memcached setting SQL: " . $sql);
    }
    return $result;
  }
  ...
  public function queryCube(DataControllerCallContext $callcontext, CubeQueryRequest $request, ResultFormatter $resultFormatter)
  {
    ...
    //@ToDo: does this need all the cube object in log?
    //LogHelper::log_debug(print_r($cube, true));
    ...
    if (isset($request->dimensions)) {
      foreach ($request->dimensions as $requestDimension) {
        ...
        $levelRootColumn = new \Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\ColumnSection($levelDatabaseColumnName);
        ...
        foreach ($requestDimension->properties as $property) {
          ...
          if (isset($levelColumnSection)) {
            ...
            $levelColumnSection = new \Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\ColumnSection($oldColumnAlias, $newColumnAlias);
    ...
    if ($cache) {
      LogHelper::log_info("Calling _checkbook_dmemcache_set with cachekey " . $cacheKey);
    ...
    switch ($datasetSourceType) {
      ...
      case DatasetTypeHelper::DATASET_SOURCE_TYPE__DYNAMIC:
        ...
        $handler = DatasetSourceAssemblerFactory::getInstance()->getHandler(
          $assembler->type,
          (isset($assembler->config) ?? $assembler->config ));
...
//    LogHelper::log_notice(t('Processed record(s)', array('@records' => $records)));
