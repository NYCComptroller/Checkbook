...
namespace Drupal\data_controller_sql\Datasource\Handler\Generator;
...
use Drupal\data_controller\Datasource\Request\Cube\__CubeQueryRequest_DimensionQuery;
...
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\AbstractConditionSection;
...
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\GroupBySection;
use Drupal\data_controller_sql\Datasource\Dataset\Assembler\Statement\HavingConditionSection;
...
use Drupal\data_controller_sql\Datasource\Handler\AbstractSQLDataSourceQueryHandler;
...
class CubeStatementGenerator extends AbstractObject
{
                ...
                foreach ($sourceDatasetPropertyQuery->values as $propertyValue) {
                    ...
                    $aggrStatement->conditions[] = new WhereConditionSection(
                        ...
                        new ExactConditionSectionValue(
                            ...
                            $propertyValue),
                        $cubeDataset->getColumn($propertyName)->type->applicationType
                    );
                }
            }
        }
        ...
        foreach ($datasetConfigs as $orderIndex => $datasetConfig) {
            ...
            if (isset($aggrSelectColumns[$orderIndex])) {
                ...
                foreach ($tableSelectColumns as $tableSelectColumn) {
                    ...
                    $aggrStatement->groupByColumns[] = new GroupBySection($attachedColumn);
                }
            }
        ...
        foreach ($link->nestedLinks as $referencedLink) {
            ...
            if (isset($referencedRequest)) {
                ...
                foreach ($referencedLink->parentColumnNames as $columnIndex => $parentColumnName) {
                    ...
                    //[$parentDimension, /* ignoring level index */] = $parentCube->getDimensionAndLevelIndexBySourceColumnName($parentColumnName);
                    ...
                    //[$referencedDimension, /* ignoring level index */] = $referencedCube->getDimensionAndLevelIndexBySourceColumnName($referencedColumnName);
                ...
                //[$isSubqueryRequired, $assembledReferencedCubeSections] = $referencedAggregationStatement->prepareSections(NULL);
    ...
    public function generateStatement(AbstractSQLDataSourceQueryHandler $datasourceHandler, DataControllerCallContext $callcontext, CubeQueryRequest $request)
    {
        ...
        //[$isSubqueryRequired, $assembledPrimaryCubeAggregationSections] = $statement->prepareSections(NULL);
