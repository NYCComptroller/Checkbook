	...
	"_fnGetDataTablesData": function ( oConfig )
	{
		...
		for ( j=0, jLen=aDataIndex.length ; j<jLen ; j++ )
		{
			...
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
                    //Customised code for checkbook
                    var sSpecific = (dt.aoColumns[i].aExportFn != undefined ? dt.aoColumns[i].aExportFn : 'display');
					/* Convert to strings (with small optimisation) */
					var mTypeData = dt.oApi._fnGetCellData( dt, aDataIndex[j], i, sSpecific );
