...
namespace Drupal\checkbook_project\NychaContractUtilities;
...
class NYCHAContractUtil {
     ...
     */
    public static function adjustYearParams($parameters) {
   ...
   */
  static public function getContractsTransactionsStaticSummary($bottomURL) {
    ...
    if (!empty($where_filter)) {
      $filter = implode(self::AND, $where_filter);
    }
    ...
    if(isset($year_id)){
      $year_filter = $year_id . " BETWEEN start_year_id" . self::AND . "end_year_id" . self::AND;
    }
    $year_filter = isset($year_id) ? $year_filter : '';
    if (($inv_tcode == 'BA' || $inv_tcode == 'BAM' || $inv_tcode == 'PA'|| $inv_tcode == 'PAM'|| $inv_tcode == 'PO') && isset($inv_contractID)) {
      $query = "SELECT contract_id, purpose, vendor_name, vendor_id,
                ...
                WHERE (" . $year_filter . " agreement_type_id =" . $agreement_type_id . self::AND  . "contract_id=" . $inv_contractID .
    ...
    if ($inv_tcode == "RESC") {
      if (isset($filter)) { $filter = $filter . self::AND; }
      if (isset($inv_respID)) {
        if (isset($inv_vendorID)) { $filter = "vendor_id =".$inv_vendorID . self::AND; }
        $query = "SELECT responsibility_center_id, responsibility_center_code, responsibility_center_name as responsibility_center_descr,
                 ...
                 WHERE (".$filter . " release_approved_year_id = " . $year_id . self::AND . "responsibility_center_code IS NOT NULL" . self::AND . "responsibility_center_id = " . $inv_respID . " )
    ...
    if ($inv_tcode == 'VO' || $inv_tcode == 'IND' || $inv_tcode == 'AWD'|| $inv_tcode == 'IND'|| $inv_tcode == 'DEP' || $inv_tcode == 'SZ') {
      ...
      $query = "SELECT " . $query_val1 .
        ...
        " , contract_id ,max(total_amount) as total_amount,
                  max(original_amount) as original_amount,
                  max(spend_to_date) as spend_to_date
                  from contracts_widget_summary WHERE (" . $year_id . " BETWEEN  start_year_id" . self::AND . "end_year_id" . self::AND
        .$filter.$dep_id.")
