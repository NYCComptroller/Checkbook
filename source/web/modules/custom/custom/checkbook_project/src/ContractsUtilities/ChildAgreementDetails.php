...
class ChildAgreementDetails {
  ...
  public function getData(&$node){
    ...
    $node->data = $results1;
    $magid = MasterAgreement::_get_master_agreement_id();
    if(!empty($magid)){
    	$magdetails = MasterAgreementDetails::_get_master_agreement_details($magid);
    	$node->magid = $magid;
   ...
   * @return array
   */
  public static function _get_child_agreement_details($agid){
    if (!isset($agid)) {
      return NULL;
    }
