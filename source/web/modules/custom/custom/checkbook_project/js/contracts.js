...
(function ($) {
  ...
  $(document).ready(function(e) {
    ...
    /*var firstTimeLoad = function (clickonload) {
      // var nTr = this.parentNode.parentNode;
      if ( jQuery(clickonload).attr('class').match('loader') ){
        //ignore
        return;
      }

      if ( !(jQuery(clickonload).attr('class').match('expanded')) ){
        jQuery(clickonload).removeClass('collapsed').addClass('loader');
        var magid = jQuery(clickonload).attr('agid');
        fnAddChildAgreementDetails( clickonload);
      }
    }*/

    var firstTimeLoad = function () {
      $(".clickOnLoad").each(function(){
        if ( jQuery(this).attr('class').match('loader') ){
          //ignore
          return;
        }
	   ...
	   //fallback for error
	    ...
	    jQuery.ajax({
	        ...
	        success: function(results) {
                ...
                //changed for ticket NYCCHKBK-12848 only getting region--content as this was
                //causing more buttons for visualixations to appear
                jQuery($container).html(jQuery('.region--content', results));
