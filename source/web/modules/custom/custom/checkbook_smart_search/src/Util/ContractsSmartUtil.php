  ...
  public static function displayContractsResult($contracts_results, $solr_datasource) {
    ...
    foreach ($contracts_parameter_mapping as $key => $title) {
      ...
      if (!$isOge && $key == "vendor_name") {
        $sub_vendor_label = !preg_match("/newwindow/", RequestUtilities::getCurrentPageUrl()) ? "<a href='https://comptroller.nyc.gov/reports/contract-primer/#subcontracts' target='_blank'>Sub Vendor</a>" : "Sub Vendor";
