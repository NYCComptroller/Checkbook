...
class QueueUtil {
  ...
  static function searchQueue($email, $request) {
    ...
    $job_id = \Drupal::database()->query($job_sql, array(':request' => $request))->fetchField(0);
    if ($job_id) {
    ...
    if (isset($email)) {
      ...
      $token = \Drupal::database()->query($token_sql, array(':email' => $email, ':request' => $request))->fetchField(0);
      if ($token) {
        $result['token'] = $token;
      }
    }
      ...
      $token = \Drupal::database()->query($token_sql, array(':request' => $request))->fetchField(0);
  ...
  static function createQueueRequest($token, $email, $job_id) {
    ...
    return \Drupal::database()->insert('custom_queue_request')->fields($fields)->execute();
  }
  ...
  static function createNewQueueRequest($queue_request) {
    ...
    $job_query = \Drupal::database()->insert('custom_queue_job')
  ...
  static function getNextJob() {
    $job_details = \Drupal::database()->query('SELECT job_id, name, data_command, request_criteria FROM {custom_queue_job} WHERE status = 0 ORDER BY created ASC LIMIT 1', array())->fetchAssoc();
  ...
  static function claimJob($job_id) {
    $db_update_query = \Drupal::database()->update("custom_queue_job")->fields(array(
      'status' => 1,
      'start_time' => time(),
    ))
  ...
  static function updateJobDetails($job_id, $job_details, $job_log) {
    ...
    $update_query = \Drupal::database()->update("custom_queue_job");
  ...
  static function isJobsInProgress() {
    $record_count = \Drupal::database()->query('SELECT count(*) recordCount FROM {custom_queue_job} WHERE status = 1', array())->fetchField(0);
  ...
  static function getRequestDetailsByToken($token) {
    ...
    $job_details = \Drupal::database()->query($sql, array(':token' => $token))->fetchAssoc();
  ...
  static function updateJobRequestEmailStatus($request_id) {
    $db_update_query = \Drupal::database()->update("custom_queue_request")->fields(array(
      'sent_email' => 'Y',
    ))
    ...
    static function updateConfirmationMailSentDate($token) {
        $db_update_query = \Drupal::database()->update("custom_queue_request")
