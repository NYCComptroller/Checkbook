...
namespace Drupal\data_controller_metamodel_file;
...
use Drupal\data_controller\Common\Path\PathHelper;
...
abstract class AbstractFileMetaModelLoader extends AbstractMetaModelLoader {
    ...
    public function load(\Drupal\data_controller\MetaModel\Factory\AbstractMetaModelFactory $factory, \Drupal\data_controller\MetaModel\Handler\AbstractMetaModel $metamodel, array $filters = NULL, $finalAttempt) {
        $metamodelTypeFolder = $this->getMetaModelFolderName();

        $filecount = 0;
        $metamodelConfigurations = \Drupal::moduleHandler()->invokeAll('dc_metamodel');
        foreach ($metamodelConfigurations as $metamodelConfiguration) {
            $path = $metamodelConfiguration['path'] . DIRECTORY_SEPARATOR.'src'. DIRECTORY_SEPARATOR. $metamodelTypeFolder . DIRECTORY_SEPARATOR . 'metadata';
