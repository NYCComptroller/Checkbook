...
use Drupal\widget_services\Common\CommonService;
...
class WidgetUtil
{
    ...
    public static function generateLabelMapping($labelAlias, bool $labelOnly = false){
        ...
        if(in_array($labelAlias,$dynamic_labelAlias)){
            ...
            if($labelOnly){
                ...
                $label  = "<div><span>". (strlen($labelAlias) > 0 ? self::$labels[$labelAlias] : "" )."</span></div>";
            }
        }
        return $label;
    }
   ...
   */
    static $labels = array(
        ...
        "distribution_line_number" => "Distribution<br/>Line Number",
   ...
   */
   public static function generateTable($table_definition, $is_main = true){
      ...
      if(isset($table_body)) {
        ...
        if (is_array($header_columns) && count($header_columns) > 0) {
          $html .= "<tr class='odd'>";
   ...
   * @return string
   */
    public static function generateHeaderColumns($header_columns)
    {
        $html = "";
        $border = "";
        $th = '<th style="';
   ...
   * @return string
   */
    public static function generateColumns($columns)
    {
        $html = "";
        $border = "";
   ...
   */
    ...
    public static function getWidgetTemplate($nid){
