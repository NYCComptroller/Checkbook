...
class payrollDetails {
    ...
    public function getData(&$node){
        if (!empty(RequestUtilities::getBottomContUrl())) {
          list ($year_type, $year, $title) = RequestUtilities::get(['yeartype', 'year|calyear', 'title'], ['q' => RequestUtilities::getBottomContUrl()]);
          list ($agency, $month, $smnid,$payroll_type) = RequestUtilities::get(['agency', 'month', 'smnid','payroll_type'], ['q' => RequestUtilities::getBottomContUrl()]);
          $current_url = explode('/',RequestUtilities::getBottomContUrl());
        } else {
          ...
          $current_url = explode('/',RequestUtilities::getCurrentPageUrl());
        }

        $url_to_check = (!empty(RequestUtilities::getBottomContUrl())) ? RequestUtilities::getBottomContUrl() : RequestUtilities::getCurrentPageUrl();
        $summary_by_agency = (strpos($url_to_check, '/payroll/agencywide/transactions/') === 0);
