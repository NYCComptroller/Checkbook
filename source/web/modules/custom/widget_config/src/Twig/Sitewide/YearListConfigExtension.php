...
public function getFunctions()
{
  ...
  public function generateYearList() {
    ...
    elseif (RequestUtilities::get('calyear')) {
      $yearParamValue = RequestUtilities::get('calyear');
    }
    elseif (str_contains($url, "contracts_pending_exp_landing") || str_contains($url, "contracts_pending_rev_landing")) {
    ...
    $yearListOptions = $fyDisplayData;

    // Calendar Year options: Required only for Payroll domain (not for NYCHA).
    if ($domain == CheckbookDomain::$PAYROLL) {
      foreach ($calendarYears as $key => $value) {
        ...
        $displayText = ($dataSource != Datasource::NYCHA ? 'CY' : 'FY').' '.$value['year_value'].' (Jan 1, '.$value['year_value'].' - Dec 31, '.$value['year_value'].')';
