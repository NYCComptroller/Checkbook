...
class RevenueCapacityTrendsCSV  {
  ...
  public static function assesedValAndEstdActValCsv($node){
    ...
    foreach ($node->data as $row) {
      ...
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['class_one'],1, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['class_two'],1, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['class_three'],1, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['class_four'],1, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['less_tax_exempt_property'],1, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['total_taxable_assesed_value'],1, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['total_direct_tax_1'],2, $dollar_sign) . '"';
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['estimated_actual_taxable_value'],1, $dollar_sign) . '"';
    ...
    $output .= "\n".'"'."             residential purposes. Except in Manhattan, Vacant land that is residentially zoned, or of not residentially zoned,".'"';
    $output .= "\n".'"'."             that abuts a class 1 parcel, has the same owner as that parcel, and is not larger than 10,000 square feet.".'"';
    ...
    $output .= "\n". '"' . "Fair Value is the worth of the property as determined by the Department of Finance based on".'"';
    $output .= "\n". '"' . "the property's tax class and the New York State Law requirements for determining fair value.".'"';
  ...
  public static function propertyTaxRatesCsv($node) {
    ...
    $header = self::FISCAL_YEAR;
    $header .=  ",Basic Rate (1)" ;
    $header .=  ",General Obligation Debt Service (1)" ;
    $header .=  ",Total Direct";
    ...
    foreach( $node->data as $row){
      ...
      $rowString .= ',' . $dollar_sign . $row['general_obligation_debt_service'];
  ...
  public static function propertyTaxLeviesCsv($node) {
    ...
    foreach( $node->data as $row){
      ...
      $rowString = $row['fiscal_year'] ;
      $rowString .= ',' . '"'. FormattingUtilities::trendsNumberDisplay($row['tax_levied'], 0, $dollar_sign).'"';
      $rowString .= ',' . '"'. FormattingUtilities::trendsNumberDisplay($row['amount'], 0, $dollar_sign).'"';
      $rowString .= ',' . '"'. $row['percentage_levy'].'"'. $percent_sign;
      $rowString .= ',' . '"'. (($row['collected_subsequent_years']>0)?FormattingUtilities::trendsNumberDisplay($row['collected_subsequent_years'], 0 , $dollar_sign):'-').'"';
      $rowString .= ',' .'"'.  FormattingUtilities::trendsNumberDisplay($row['levy_non_cash_adjustments'], 0, $dollar_sign).'"';
      $rowString .= ',' . '"'. FormattingUtilities::trendsNumberDisplay($row['collected_amount'], 0, $dollar_sign).'"';
      $rowString .= ',' . '"'. $row['collected_percentage_levy'].'"'. $percent_sign;
      $rowString .= ',' . '"'. FormattingUtilities::trendsNumberDisplay($row['uncollected_amount'], 0, $dollar_sign).'"';
  ...
  public static function collectionsCancellationsAbatementsCsv($node) {
    ...
    foreach( $node->data as $row){
      ...
      $rowString = $row['fiscal_year'] ;
      $rowString .= ',' . '"' . FormattingUtilities::trendsNumberDisplay($row['tax_levy'],1, $dollar_sign).(($row['fiscal_year']=='2003')?'(2)':'') . '"';
  ...
  public static function uncollectedParkingViolationFeeCsv($node) {
    ...
    foreach( $table_rows as $row){
      ...
      $rowString = '"'.$row['category'].'"' ;
      foreach ($years as $year){
        $rowString .=  ',' . '"'. (($row[$year]['amount'] >0) ?FormattingUtilities::trendsNumberDisplay($row[$year]['amount'], 0, $dollar_sign) : '') .'"';
      }
      $output .= $rowString . "\n";
      $count++;
    }
  ...
  public static function hudsonYardsInfraCorpCsv($node) {
    ...
    $header .=  ",Coverage on Total Revenue(6)(7)";
    ...
    foreach($node->data as $row){
      ...
      $rowString = $row['fiscal_year'] ;
      $rowString .=  ',' .(($row['dib_revenue_1']<>0)? ('"'.FormattingUtilities::trendsNumberDisplay($row['dib_revenue_1'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['tep_revenue_2']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['tep_revenue_2'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['isp_revenue_3']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['isp_revenue_3'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['pilomrt_payment']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['pilomrt_payment'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['pilot']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['pilot'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['other_4']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['other_4'], 0, $dollar_sign).'"'):'-').(in_array($row['fiscal_year'], ['2019']) ? '(8)' : '');
      ...
      $rowString .= ',' .(($row['principal']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['principal'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['total']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['total'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' .(($row['operating_expenses']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['operating_expenses'], 0, $dollar_sign).'"'):'-') . ($row['fiscal_year'] == '2012' ? '(8)' : '');
      $rowString .= ',' .(($row['total_to_be_covered']<>0)?('"'.FormattingUtilities::trendsNumberDisplay($row['total_to_be_covered'], 0, $dollar_sign).'"'):'-');
      $rowString .= ',' . (($row['coverage_on_total_revenue_5'] > 0) ? $row['coverage_on_total_revenue_5'] : "(" . $row['coverage_on_total_revenue_5'] .")");
    ...
    $output .= "\n".'"'."(8) In December 2011, HYIC was obligated to make an arbitrage rebate payment to United States Treasury for $8.8M. In February 2019, the $8.8M payment was refunded back to HYIC. ".'"';
