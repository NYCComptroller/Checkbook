...
class ContractURLHelper{
    ...
    public function prepareActRegContractsSliderFilter($page, $status){
        ...
        if( preg_match("/^contracts_pending/", drupal_get_path_alias($_GET['q'])) ){
          $allowedFilters =  array("agency","vendor","awrdmthd","csize","cindustry","agid","dashboard","subvendor");
          $url .= "/yeartype/B/year/"._getCurrentYearID();
        }
        else{
          $allowedFilters =  array("year","calyear","agency","yeartype","awdmethod","vendor","csize","cindustry","agid","dashboard","subvendor");
        }
