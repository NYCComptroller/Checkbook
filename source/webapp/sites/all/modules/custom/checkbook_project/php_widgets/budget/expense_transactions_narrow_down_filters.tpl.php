...
*/
    ...
    if(isset($reqAgencyId) && count($departments) > 0){
        ...
        foreach($departments as $key=>$value){
            ...
            if($i<5){
                $displayRows .= "<tr><td><input type='checkbox' name='fdeptId' {$checked} value='{$value['dept_dept']}' onClick='javascript:applyTableListFilters();'/></td>  <td>{$deptName}</td> <td>{$value['txcount']}</td> </tr>";
            }else{
                $hiddenRows .= "<tr><td><input type='checkbox' name='fdeptId' {$checked} value='{$value['dept_dept']}' onClick='javascript:applyTableListFilters();'/></td>  <td>{$deptName}</td> <td>{$value['txcount']}</td> </tr>";
            }
            $i++;
        }
