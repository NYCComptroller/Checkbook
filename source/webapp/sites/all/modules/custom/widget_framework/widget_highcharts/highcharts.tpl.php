...
<?php if(($node->totalDataCount == 0 && isset($node->totalDataCount) ) || (is_countable($node->data) && count($node->data) == 0) ) {  ?>
...
jQuery(document).ready(function() {
  var chart = new Highcharts.Chart(<?php
      print widget_mergeJSFunctions($node, $node->widgetConfig->chartConfig);
      if ($node->widgetConfig->callback){
          print ',function(chart){'.$node->widgetConfig->callback.'}';
      }?>);
