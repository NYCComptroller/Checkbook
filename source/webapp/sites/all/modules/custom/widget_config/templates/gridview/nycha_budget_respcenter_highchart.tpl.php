...
include_once('nycha_budget_title.php');
...
<table id="table_<?php echo widget_unique_identifier($node) ?>" class="<?php echo $node->widgetConfig->gridConfig->html_class ?>">
    <thead>
    <?php
        echo "<tr><th class='text'>" . WidgetUtil::generateLabelMapping("responsibility_center") . "</th>
...
<?php
        if (isset($node->data) && is_array($node->data)) {
            foreach ($node->data as $datarow) {
                echo "<tr>";
                   echo '<td>' . $datarow['responsibility_center_description_responsibility_center_description'] . '</td>';
                        ...
                        "aoColumnDefs": [
                            {
                                ...
                                "mDataProp": function ( source, type, val ) {
                                    if (type == "set") {
                                        source.responsibility_center_description_responsibility_center_description = val;
                                        source.responsibility_center_description_responsibility_center_description = "<div>" + val + "</div>";
                                        return;
                                    }else if (type == "display") {
                                        return source.responsibility_center_description_responsibility_center_description;
                                    }else if (type == "sort") {
                                        return source.responsibility_center_description_responsibility_center_description;
                                    }
                                    return source.responsibility_center_description_responsibility_center_description;
