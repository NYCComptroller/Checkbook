...
<table id="table_<?php echo widget_unique_identifier($node) ?>" class="<?php echo $node->widgetConfig->gridConfig->html_class ?>">
    <thead>
    <?php
        echo "<tr><th class='text'>" . WidgetUtil::generateLabelMapping("program") . "</th>
...
<?php
        if (isset($node->data) && is_array($node->data)) {
            foreach ($node->data as $datarow) {
                echo "<tr>";
                   echo '<td>' . $datarow['program_phase_description_program_phase_description'] . '</td>';
                        ...
                        "aoColumnDefs": [
                            {
                                ...
                                "mDataProp": function ( source, type, val ) {
                                    if (type == "set") {
                                        source.program_phase_description_program_phase_description = val;
                                        source.program_phase_description_program_phase_description = "<div>" + val + "</div>";
                                        return;
                                    }else if (type == "display") {
                                        return source.program_phase_description_program_phase_description;
                                    }else if (type == "sort") {
                                        return source.program_phase_description_program_phase_description;
                                    }
                                    return source.program_phase_description_program_phase_description;
