 ...
 */
function checkbook_preprocess_html(&$vars) {
 ...
 */
function checkbook_nice_menus_menu_item_link($variables) {
 ...
 */
function checkbook_css_alter(&$css) {
  // Installs the jquery.ui themeroller theme to the theme.
  if (isset($css['misc/ui/jquery.ui.theme.css'])) {
    $css['misc/ui/jquery.ui.theme.css']['data'] = drupal_get_path('theme', 'checkbook') . '/css/jquery-ui/jquery-ui-1.8.17.custom.css';
  }
}
...
function checkbook_pager_first($variables) {
  $text = $variables['text'];
...
function checkbook_pager_previous($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
...
function checkbook_pager_next($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
  $interval = $variables['interval'];
  $parameters = $variables['parameters'];
...
function checkbook_pager_last($variables) {
  $text = $variables['text'];
  $element = $variables['element'];
  $parameters = $variables['parameters'];
  global $pager_page_array, $pager_total;
...
function checkbook_pager($variables) {
  $tags = $variables['tags'];
  $element = $variables['element'];
  $parameters = $variables['parameters'];
...
function checkbook_pager_link($variables) {
  $text = $variables['text'];
 ...
 */
function checkbook_select($variables) {
  $element = $variables['element'];
  element_set_attributes($element, array('id', 'name', 'size'));
  _form_set_class($element, array('form-select'));
  return '<select' . drupal_attributes($element['#attributes']) . '>' . checkbook_form_select_options($element) . '</select>';
}
/*
 * Helper function for checkbook_select
 *
 * Retains functionality of _form_select_options(), but with additional functionality to add other attributes
 * to <option> tags.
 */
function checkbook_form_select_options($element,$choices=NULL){
  if (!isset($choices)) {
  ...
  foreach ($choices as $key => $choice) {
    if (is_array($choice)) {
      ...
      $options .= checkbook_form_select_options($element, $choice);
      $options .= '</optgroup>';
    }
    elseif (is_object($choice)) {
      $options .= checkbook_form_select_options($element, $choice->option);
    }
    else {
      $key = (string) $key;
